...
# Load ini file
source("/Users/saraknox/Code/flux_shiny_plots/ini files/UQAM_ini.R")
...
sites <- c("BB2","DSM","RBM") #unique(sapply(strsplit(dir_sites, "/"), '[[', 8))
...
# Path & variables for diagnostics tab
level_diag <- c("Flux","monitorSites") # ADD BACK IN MET (and deal with site specific issues)
tv_input_diag <- "Clean_tv"
...
data_diagnostics <- function(data,site,var_categories){
  
  # Select a subset of diagnostic variables (to update once we finalize CARBONIQUE program)
  #vars_voltage <- c('SYS_CR1000_Batt_Volt_Avg','SYS_PBox_Batt_Volt_Avg','SysCR1000BV_1_1_1','SysPBoxBattVolt_1_1_1')
  #vars_current <- c('SYS_Batt_DCCurrent_Avg','SysBattDCC_1_1_1')
  #vars_sys_temp <- c('SYS_PanelT_CR1000_Avg','SYS_PanelT_AM25T_Avg','SYS_chargerTC_Avg','SYS_BatteryBoxTC_Avg','MET_HMP_T_350cm_Avg','MET_CNR4_TC_Avg')
  vars_SS <- c('avg_signal_strength_7200_mean','rssi_77_mean','co2_signal_strength_7200_mean','h2o_signal_strength_7200_mean')
  vars_flow <- c(paste0(site,'.flowRate.avg'),paste0(site,'.flowRate.max'),paste0(site,'.flowRate.min'),
                 paste0(site,'.flowDrive.avg'),paste0(site,'.flowDrive.max'),paste0(site,'.flowDrive.min'))
  vars_samples <- c('file_records','used_records') 
  vars_7200_thermo <- c(paste0(site,'.tempIn.avg'),paste0(site,'.tempIn.max'),paste0(site,'.tempIn.min'),
                        paste0(site,'.tempOut.avg'),paste0(site,'.tempOut.max'),paste0(site,'.tempOut.min'))
  vars_7200_pressure <- c(paste0(site,'.Phead.avg'),paste0(site,'.Phead.max'),paste0(site,'.Phead.min'))
  
  vars_all_diag <- c("vars_SS","vars_flow","vars_samples","vars_7200_thermo","vars_7200_pressure") #c("vars_voltage","vars_current","vars_sys_temp","vars_SS","vars_flow","vars_samples","vars_7200_thermo","vars_7200_pressure")
  
  data <- data[,c("datetime",vars_SS,vars_flow,vars_samples,vars_7200_thermo,vars_7200_pressure)] #data[,c("datetime",vars_voltage,vars_current,vars_sys_temp,vars_SS,vars_flow,vars_samples,vars_7200_thermo,vars_7200_pressure)]
  
  # Rename variables based on categories
  for (i in 1:length(vars_all_diag)){
    vars <- eval(parse(text = vars_all_diag[i]))
    
    names(data) <- ifelse(names(data) %in% names(data[vars]),
                          paste(var_categories[i], colnames(data[vars]), sep="."),
                          names(data))
  }
...
# Load full list of diagnostic variables <-  from first site when app initializes
data_diag <- read_data_all_years(basepath,yrs,site,level_diag,tv_input_diag)

# select subset of diagnostic variables and get units
var_categories <- c('Voltages','Power_System_Current','System_Temperatures','Gas_Analyzer_Signal_Strengths','LI_7200_Flow_Rate_and_Flow_Drive','File_Record number',
                    'LI_7200_Thermocouples','LI_7200_Head_Pressure')

if (exists("data_diag")) { # Remove?
  data_diag_sub <- data_diagnostics(data_diag,site,var_categories)
  data_units_diag <- var_units(colnames(data_diag_sub),UnitCSVFilePath)
}
                      ...
                      tabItems(
                        tabItem( # Begin 'Individual Sites' page
                          ...
                          tabsetPanel( # Begin tab panels section
                            ...
                            tabPanel( # Begin diagnostics tab
                              "Diagnostics",
                              
                              br(),
                              
                              # Select main variable
                              fluidRow( column(6, selectInput('diagcol', 'Variable', var_categories)),
                              ...
                              # Ouput time series plot with a spinner
                              shinycssloaders::withSpinner(plotlyOutput('diagnostics_plots'),
                                                           type = getOption("spinner.type", default = 5),
                                                           color = getOption("spinner.color", default = "#4D90FE")),
                              h5(em('Plot shows 30-min data'), align = 'center')
...
server <- function(input, output, session) { # Begin server
  ...
  observeEvent(input$sites, { # Change output based on site selection
    ...
    # Load full list of diagnostic variables <-  from first site when app initializes
    data_diag <- read_data_all_years(basepath,yrs,input$sites,level_diag,tv_input_diag)
    
    # select subset of diagnostic variables and get units
    if (exists("data_diag")) { # Remove?
      data_diag_sub <- data_diagnostics(data_diag,input$sites,var_categories)
      data_units_diag <- var_units(colnames(data_diag_sub),UnitCSVFilePath)
    }
    ...
    # Update Y variable (diagnostics)
    updateSelectInput(session, 'diagcol', choices = var_categories)
