...
### Daily mean time series of single flux variable by year
...
renderPlotly({
  ...
  conv_fact <- 44.01/(10^6)*(60*30) # Conversion factor to account for the number of gCO2 per 30 min
  ...
  colnames(df)[3] <- "var"
   ...
   daily.cum <- df %>%
    group_by(year,DOY) %>%
    dplyr::summarize(var = sum(var*conv_fact), # Deal with the conversion factor
                     DOY = first(DOY),
                     year = as.factor(first(year)))
...
### Mean diurnal pattern - update to use non-gap-filled data!!
...
renderPlotly({
  ...
  colnames(df)[2] <- "var"
   ...
   data.diurnal <- df %>%
      ...
      group_by(year, HHMM) %>%
      dplyr::summarize(var = median(var))
  ...
  p <- ggplot() +
    ...
    scale_x_datetime(breaks="6 hours", date_labels = "%R") + ylab(input$fl3)
