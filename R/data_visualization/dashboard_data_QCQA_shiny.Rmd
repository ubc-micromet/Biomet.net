...
dataset_thirdstage <- reactive({
  data_thirdstage[, c("datetime", input$f3)]
})

renderPlotly({

  p <- ggplot() +
    geom_line(data = dataset_thirdstage(), aes_string(x = "datetime", y = input$f3),color = "black")+
    geom_line(data = dataset_flux(), aes_string(x = "datetime", y = input$f),color = rgb(0.2, 0.6, 0.9, 1))+
    geom_line(data = data_thirdstage, aes_string(x = "datetime", y = input$f),color = "#69b3a2")+
    xlab("") #ADD LEGEND
...
### daily mean time series of single flux variable
...
renderPlotly({
  ...
  p <- ggplot() +
    geom_line(data = daily.data , aes(x = DOY, y = var))+ ## ADD YEAR
    ylab(input$fl3)
  p <- ggplotly(p) %>% toWebGL()
