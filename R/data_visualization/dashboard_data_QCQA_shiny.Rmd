...
dataset <- reactive({
  data[, c("datetime", input$y)]
  diurnal.summary(data, colnames(data)[2], 30, 48)
})
 ...
 # diurnal.summary.composite$time <- as.POSIXct(as.character(diurnal.summary.composite$HHMM), format="%R", tz="UTC")
  ...
  plot_ly(data = diurnal.summary, x = ~time, y = ~diurnal.summary[[input$y]], type = 'scatter', mode = 'lines') %>%
  layout(yaxis = list(title = input$y)) %>%
    toWebGL() 
})
```
