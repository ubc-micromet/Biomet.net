...
class PointSampleNARR():
    ...
    def __init__(self,Site_code,Years,verbose=0):
        self.ini = configparser.ConfigParser()
        self.ini.read('../MicrometPy.ini')
        self.ini.read('configuration.ini')
        self.ini.read(f'../site_configurations/{Site_code}.metadata')
        self.ini.read(f'../site_configurations/{Site_code}.ini')
        ...
        Vars = self.ini['Downloads']['var_names'].split(',')
        
        if self.ini['Outputs']['datadump']=='True':
            self.out_dir = self.ini['Paths']['datadump'].replace('SITE',self.Site_code)+'/'+self.ini['Outputs']['folder_name']
        else:
            self.out_dir = self.ini['Outputs']['datadump'].replace('SITE',self.Site_code)+'/'+self.ini['Outputs']['folder_name']
        ...
        for self.var_name in Vars:
            if os.path.isfile(f'{self.out_dir}/{self.var_name}.csv'):
                self.Trace = pd.read_csv(f'{self.out_dir}/{self.var_name}.csv',
                                parse_dates=[self.ini['CSV']['timestamp']],
                                index_col=self.ini['CSV']['timestamp']
                                )
            else:
    ...
    def estimate_values(self,pad = 2,freq = '30T'):
        ...
        m.save(f'{self.ini["Downloads"]["nc_path"]}/{self.Site_code}_{self.var_name}_grid_pts.html')
        ...
        TS = pd.DataFrame()
        TS[self.ini['CSV']['timestamp']] = pd.to_datetime(self.time)+timedelta(hours=int(self.ini['Site']['utc_offset']))
        
        TS[self.var_name]=np.nan
        for i,row in TS.iterrows():
            TS.loc[TS.index == i, self.var_name] = self.interpolate(self.var_clip[i].flatten())
        TS = TS.set_index(self.ini['CSV']['timestamp'])
