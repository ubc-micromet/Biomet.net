function [NEP,R,GEP,NEPgf,Rgf,GEPgf,RHat,GEPHat,RHat0,GEPHat0,cR,cGEP] = FCRN_CO2Flux2NEP_MB_MovWin(t,NEE,uStar,PPFD,Ta,Ts,PPFDGF,TaGF,TsGF,isNight,uStarTH,FracSEBClosure,LogFile,Plots) 
...
%	First, fit the bR parameters using binned means of all of the data. 
	...
	nReg 					= length(iReg); 
	bRGuess	 			= [20 0.10 20]; 
	[bR,b,c] 					= FCRN_function_fit(fTs2R,bRGuess,R(iReg),[],Ts(iReg));  %#ok<ASGLU>
	RHat0 				= fTs2R(bR,TsGF); 
...
%	are first fit to each window and then interpolated to (cR(t),t). 
    ...
    for jReg1 = 1:WindowIncR:nReg 
        ...
        if jReg2<=nReg 
            ...
            cDiary = char(fread(fid))';  %#ok<FREAD>
...
%	nReg_winter  					= length(iReg_winter ); 
	...
	[bR_winter ,b_winter ,c_winter ] 					= FCRN_function_fit(fTs2R,bRGuess_winter ,R_winter(iReg_winter ),[],Ts(iReg_winter ));  %#ok<ASGLU>
...
%	First, estimate the constant bGEP parameters using warm-season data only. 
	...
	bGEPGuess 			= [20 500]; 
	[bGEP,b,c]					= FCRN_function_fit(fPPFD2GEP,bGEPGuess,GEP(iReg),[],PPFD(iReg,:));  %#ok<ASGLU>
...
%	Optional log file.
    ...
    if ~isempty(LogFile) 
        ...
        [myR,mmR,mdR,mhR,mnR,msR]=FCRN_datevec(mtR);  %#ok<ASGLU>
        mmddR=mmR*100+mdR; hhmmR=mhR*100+mnR; 
        doyR=FCRN_DOY(mtR); 

        [myGEP,mmGEP,mdGEP,mhGEP,mnGEP,msGEP]=FCRN_datevec(mtGEP); %#ok<ASGLU>
        ...
        [y,m,d,h,mn,s]=FCRN_datevec(t); iYr=median(y); iy=find(y==iYr);  %#ok<ASGLU>
%         mmdd=m*100+d; 
%         hhmm=h*100+mn; 
...
%	5. GEP vs PPFD. 
    ...
    if ismember(5,Plots) 
        figure(5); 
        clf; % (PPFD {Ta D],GEP)
        plot(PPFD(iNotColdSeason),GEP(iNotColdSeason),'k.'); 
        hold on; 
        plot(PPFD(iNotColdSeason),GEPHat(iNotColdSeason),'r.'); 
        hold on; 
        set(gca,'FontSize',12,'FontWeight','bold'); 
        grid on; 
        hold off; 
    ...
    if ~isempty(Plots) 
        disp('Paused after CO2Flux2NEP plots.'), 
        pause; 
    end 
