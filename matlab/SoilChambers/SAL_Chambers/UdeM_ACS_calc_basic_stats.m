...
%
% Aug 29, 2023 (Zoran)
%   - "LGR" was hard-coded in the program. Made it more generic
%
...
for chNum = 1:configIn.chNbr
    % do this for all analyzers
    aNames = fields(indOut.analyzer);
    for cntAnalyzer = 1:length(aNames)
        analyzerName = char(aNames{cntAnalyzer});
        % cycle through all the samples (chamber runs, usually 24 per day)
        for sampleNum = 1:length(indOut.analyzer.(analyzerName)(chNum).start)
