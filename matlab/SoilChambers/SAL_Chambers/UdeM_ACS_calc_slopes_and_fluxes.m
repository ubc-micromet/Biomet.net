...
%
%  Aug 29, 2023 (Zoran)
%    - "LGR" was hard coded as in ".analyzer.LGR". Made it generic BUT it will NOT work if 
%      more than one analyzer are used with the system.
%    - programmed in automatic conversion from default gas names (first column in configIn.chamber.traces() matrix)
%      (see use of the function findRawTraceName).
%    - added option for gasType = n2o
%  Apr  20, 2023 (Zoran)
%    - added global fit parameters:
%             optionsIn.functionTolerance = configIn.globalFitParam.functionTolerance.(gasType);
%             optionsIn.rmse_envelope_percent = configIn.globalFitParam.rmse_envelope_percent;
%             optionsIn.useOriginalFit    = configIn.globalFitParam.useOriginalFit;
% Apr 1, 2021 (Zoran)
...
%
    ...
    % Analyzer name
    strAnalyzerName = char(fields(dataStruct.rawData.analyzer));
    ...
    for chNum=chamberList
        ...
        % Find CO2 raw trace
        strRawCO2 = findRawTraceName(configIn,chNum,'co2_dry');
        traceY = medfilt1(dataStruct.rawData.analyzer.(strAnalyzerName).(strRawCO2),3);   % filter out the spikes
        %
...
%                 dataStruct = UdeM_data_fit(dataStruct,chNum,sampleNum,'ch4','fit_quad',oneIteration,flagVerbose);            
                ...
                if plotFlag
                    ...
                    t_fit = ch_time_hours_analyzer(indX(sampleNum):indY(sampleNum));
...
function dataStruct = UdeM_data_fit(dataStruct,chNum,sampleNum,gasType,fitType,useLicorMethod,flagVerbose)
    ...
    switch lower(gasType)
        case 'co2'
            strRawTraceName = findRawTraceName(configIn,chNum,'co2_dry');
            traceY = medfilt1(dataStruct.rawData.analyzer.(strAnalyzerName).(strRawTraceName),3);   % filter out the spikes
        case 'ch4'
            strRawTraceName = findRawTraceName(configIn,chNum,'ch4_dry');
            traceY = medfilt1(dataStruct.rawData.analyzer.(strAnalyzerName).(strRawTraceName),3);   % filter out the spikes
        case 'n2o'
            strRawTraceName = findRawTraceName(configIn,chNum,'n2o_dry');
            traceY = medfilt1(dataStruct.rawData.analyzer.(strAnalyzerName).(strRawTraceName),3);   % filter out the spikes
    end
    tv = dataStruct.rawData.analyzer.(strAnalyzerName).tv;
    ...
    optionsIn.functionTolerance     = dataStruct.configIn.globalFitParam.functionTolerance.(gasType);
    optionsIn.rmse_envelope_percent = dataStruct.configIn.globalFitParam.rmse_envelope_percent;
    optionsIn.useOriginalFit        = dataStruct.configIn.globalFitParam.useOriginalFit;

    switch lower(fitType)
        case 'fit_exp'        
            [fitOut,fCO2,gof] = ...
                             UdeM_exponential_fit(t_fit/24,c_fit,optionsIn,useLicorMethod,gasType,[],optionsIn.useOriginalFit);
            if useLicorMethod
                outputType = 'exp_L';
            else
                outputType = 'exp_B';
