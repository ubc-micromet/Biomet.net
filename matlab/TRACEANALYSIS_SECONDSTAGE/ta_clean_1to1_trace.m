...
%
% For two traces that are known to be linearly dependan,
% this program will find the outliers that are more than stdMultiplier * standard deviation of residuals
% times further away from the linear fit line. 
%
% NOTE:
%     - The program is designed to work assuming that both traces are being 
%       continuous and that the only thing that matters for this cleaning 
...
%       *will be wiped* too before the line fit is calculated!
%     - One robust way to use this function inside of TraceAnalysis ini files would be:
%           - run this program only if both traces have more than 80% of
%             not-nan values (data coverage> 80%)
...
%                                   Last modification:  Aug 8, 2022

%
% Revisions:
%
% Aug 8, 2022 (Zoran)
%       - the program used to calculate residual error as:
%           residualError = abs(x*slopeCoeff-y )
%         This biased the results. The proper thing to do is:
%           residualError = abs((polyval(p1,x)-y) )
% May 13, 2020 (Zoran)
...
residualError = abs((polyval(p1,x)-y) ) ;                % calculate residual error of fit (asume 1.9 gain)
thresholdFilter = stdMultiplier * std(residualError);   % calculate the filter threshold 
ind =  residualError< thresholdFilter;          % find index of "good" points
