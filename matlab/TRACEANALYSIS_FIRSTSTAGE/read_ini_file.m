...
endYearDate = datenum(Year,1,1,0,0,0);
for cntTrace = 1:length(trace_str)
    ...
    if bool_no_inputFileName_dates 
        ...
        for cntRows = 1:size(datesMatrix,1)
            ...
            % trace
            if   (strYearDate >= datesMatrix(cntRows,1) && strYearDate <= datesMatrix(cntRows,2)) ...
              || (endYearDate >= datesMatrix(cntRows,1) && endYearDate <= datesMatrix(cntRows,2))
                bool_validTrace = 1;
                break
            end
        end
