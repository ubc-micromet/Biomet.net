...
if strcmpi(file_opts.out_path, 'database')
       ...
       switch upper(trc.stage)
           case 'FIRST'
              switch upper(trc.ini.measurementType)
                  case {'CL' 'BERMS'}
                     pathOut = [file_opts.out_path 'climate\clean\'];
                  case 'FL'
                     pathOut = [file_opts.out_path 'flux\clean\'];
                  case 'PR'
                     pathOut = [file_opts.out_path 'profile\clean\'];
                  case 'CH'
                     pathOut = [file_opts.out_path 'chambers\clean\'];
                  otherwise
                     pathOut = [file_opts.out_path '\' trc.ini.measurementType '\clean\'];
               end
           otherwise
    ...
    elseif strcmp(file_opts.format,'bnr')
       switch upper(trc.ini.measurementType)
            case 'CL'
                pathOut = [file_opts.out_path 'climate\'];
            case 'FL'
                pathOut = [file_opts.out_path 'flux\'];
            case 'PR'
                pathOut = [file_opts.out_path 'profile\'];
            case 'CH'
                pathOut = [file_opts.out_path 'chambers\clean\'];
            otherwise
                pathOut = [file_opts.out_path];
...
if ~strcmpi(file_opts.out_path, 'database')
   if isfield(trc,'data') & ~isempty(trc.data) & length(trc.data)>1
       ...
       case 'bnc' 
           ...
           save_bor([pathOut trc.variableName],1,export_data);
           ...
           if length(clean_tv) == 17520 | length(clean_tv) == 17568 %#ok<*OR2>
              save_bor([pathOut 'clean_tv'],8,clean_tv);
           % 3min data for FSP project (PAR data)
           elseif  length(clean_tv) == 175200 | length(clean_tv) == 175680
              save_bor([pathOut 'clean_3min_tv'],8,clean_tv);
           % 10min data for Mark Johnson's WaterQ system
           elseif length(clean_tv) == 52560 | length(clean_tv) == 52704
              save_bor([pathOut 'clean_10min_tv'],8,clean_tv);
           % hourly data for UBC Totem historical data
           elseif length(clean_tv) == 8760 | length(clean_tv) == 8784 
              save_bor([pathOut 'clean_hourly_tv'],8,clean_tv);
              save_bor([pathOut 'hourly_clean_tv'],8,clean_tv);
           end
       case 'bnr'
           if isfield(trc,'data_old')
               save_bor([pathOut  trc.variableName],1,trc.data_old(ind_tv));
               save_bor([pathOut 'clean_tv'],8,clean_tv(ind_tv));
           else
