...
%
    ...
    while cntFiles < length(s_all_new_files)
            ...
            try
                ...
                if any(diffNans) | ~all(x_new(~nansInNew)-x_old(~nansInNew)==0)  %#ok<*AND2>
                    ...
                    if isempty(fig)
                        ...
                        hButton = uibutton(fig,'push',...
                            'Text','Next >>',...
                            'Position',[20, 184, 100, 40],...
                            'ButtonPushedFcn', @(hButton,event) nextButtonPushed(hButton,fig));
                    else
        ...
        if flagDiffFound > 0
            ...
            UserData = get(fig,'UserData');
            cntFiles = cntFiles + UserData.cnt;
            badN = badN + UserData.cnt;
            % set
        end
        cntFiles = cntFiles + 1;
    end
