...
%                               Last modifications: Mar 14, 2022
    ...
    if ~exist(currentFile,'dir')
        try
            ...
            if any(diffNans) | ~all(x_new(~nansInNew)-x_old(~nansInNew)==0)  %#ok<*AND2>
                ...
                plot(doy,x_new,'linewidth',2,'color','#0072BD')
                
                hold on
                plot(doy,x_old,'linewidth',1,'color','#D95319')
                plot(doy(indDiff),x_new(indDiff),'og','MarkerSize',8)
                % The above will not plot NaNs in x_new. So plot 'x' at 0 for
                % those
                x_new_isnan = find(isnan(x_new(indDiff))& ~isnan(x_old(indDiff)));
                plot(doy(indDiff(x_new_isnan)),zeros(size(indDiff(x_new_isnan))),'xg','MarkerSize',8)
                hold off
                grid on
                if ~isempty(x_new_isnan)
                    legend({['NEW data=>' path_new],['OLD data=>' path_old],'Different new points','NaNs only in NEW'},'Interpreter','none')
                else
                    legend({['NEW data=>' path_new],['OLD data=>' path_old],'Different new points'},'Interpreter','none')
                end
                title(sprintf('Year: %d       Trace: %s;      # of diff: %d ',...
                       yearX, s_all_new_files(i).name,length(indDiff)),...
                       'Interpreter','None')
