...
tm = datevec(now);
gs = FCRN_gapsize(nep);
if tm(1) == Xyear | max(gs>672) %#ok<*OR2>
    ind_future = t > fr_get_doy(now) & tm(1) == Xyear;
    ind_nodata = t(gs>672) & tm(1) ~= Xyear; %no data for two weeks in a row assumed to catch years of site establishment
...
ind_dub = isnan(nep); flag_dub = zeros(length(t),1); flag_dub(ind_dub) = 1;
...
ind_dub = isnan(er); flag_dub = zeros(length(t),1); flag_dub(ind_dub) = 1;
bad_data = find(flag_future==0 & flag_nodata==0 & flag_dub==1);
if length(bad_data)>0
    temp = FCRN_nanmean(er(er>=0));
    er(bad_data) = temp;
...
ind_dub = isnan(psn); flag_dub = zeros(length(t),1); flag_dub(ind_dub) = 1;
bad_data = find(flag_future==0 & flag_nodata==0 & flag_dub==1);
if length(bad_data)>0 %#ok<*ISMT>
    temp = FCRN_nanmean(psn(psn>=0));
    psn(bad_data) = temp;
...
if style_ == 1
    npp_trace = [cumsum(npp(ind_data))*1800*0.000012011]; %#ok<*NBRAK> %convert umol CO2/m2/s to g C/m2/yr
else
...
%output the rest
ind_bad         = a_mean==0; %nighttime is not the right time... (r is undefined at night and therefore so is a)
a_mean(ind_bad) = NaN;
...
if figure_ == 1
    ...
    xlabel('Day of year');
    legend('NEP', 'R_{E}', 'P', 'NPP','R_{a}','R_{h}','northwest');
