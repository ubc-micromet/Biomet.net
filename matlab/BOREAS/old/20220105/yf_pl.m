...
if datenum(year(end),1,0+t(end)) < datenum(2019,07,24,0,0,0)
    BB1  = read_sig( [fr_logger_to_db_fileName(ini_climMain, 'Main_V_AVG', pth)], ind,year, t, 0 );
else
    BB1 = read_sig( fullfile(pth,'YF_Snow', 'YF_Snow.31'), ind,year, t, 0 );
    BB2 = read_sig( fullfile(pth,'YF_Snow', 'YF_Snow.33'), ind,year, t, 0 );
end
trace_path  = (Ibb) .* BB1;
trace_legend = [];
trace_units = '(W)';
...
if datenum(year(end),1,0+t(end)) < datenum(2019,07,24,0,0,0)
    ...
    Ibb1 = read_sig( fullfile(pth,'YF_Snow', 'YF_Snow.32'), ind,year, t, 0 );
    Ibb2 = read_sig( fullfile(pth,'YF_Snow', 'YF_Snow.34'), ind,year, t, 0 );
...
x = plt_msig( [cumsum(x1) cumsum(x2) cumsum(x3)+addRain], ind, trace_name, trace_legend, year, trace_units, y_axis, tx_new, fig_num);
