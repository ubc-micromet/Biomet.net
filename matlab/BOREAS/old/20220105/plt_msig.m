function x = plt_msig( pth, ind, trace_name, trace_legend, year, trace_units, y_axis, t, fig_num,coeff,offset )
%
...
%                               Last modification:  Jan  5, 2022
%

% Revisions:
% 
%  Jan 5, 2022 (Zoran)
%   - Introduced datetime plotting
%   
%  Apr 9, 2020 (Zoran)
%   - Matlab 2014-> compatibility updates
%  Feb  4, 2011 (Zoran)
...
dtv = datetime(datenum(year,1,t),'convertfrom','datenum'); % datetime vector
...
try
    ...
    %LineTypes = char('y-','r--','g-.','m:','b-','y--','r-','g:','m-','b--');
    LineTypes = char('-','-','-','-','-','-','-','--','--','--','--','--','--',':',':',':',':',':',':',':');
    ...
    for i=1:N
        ...
        tmp(find(abs(tmp)>1e10)) = NaN;  %#ok<*FNDSB>
        timeVector = datetime(datenum(year,1,tx),'convertfrom','datenum');
        plot(timeVector,tmp.*coeff(i) - offset(i),LineTypes(kk,:),'linewidth',lineWidth)
        hold on
    ...
    line(dateshift([dtv(end)-1 dtv(end)-1],'end','day'),...
    ...
    clf
    ax = xlim;
    text(ax(1)+0.03,0.5,'Error while plotting (function: plt\_msig.m)');
    if exist('i','var') & exist('pth','var')
        text(ax(1)+0.03,0.4,deblank(pth(i,:)));
    end
    text(ax(1)+0.03,0.3,'Error message:');
    xxx = ME;
    text(ax(1)+0.03,0.2,['"' xxx.message '"' ]);
end