...
switch upper(SiteID)
    ...
    case 'YF'
        ...
        trace_path  = str2mat('\\annex001\database\2019\Yf\Climate\YF_CR1000_1_MET_30\MET_CNR1_Net_Avg');
...
if ~(strcmp(upper(SiteID),'YF') & st+startDate-1 >= datenum(2018,1,1,0,0,0))
...
if strcmp(upper(SiteID),'YF') & datenum(year,1,st)>=datenum(2015,7,9,18,0,0);
    ...
    grid on;zoom on;xlabel('DOY')
    title({'Eddy covariance: ';'Friction Velocity'});
    set_figure_name(SiteID)
    ylabel('m/s');
    legend('Sonic');
    zoom on;
...
%===========================================
    ...
    if ~strcmp(SiteID,'YF') | (strcmp(SiteID,'YF') & (st+startDate-1 >= datenum(2018,1,1,0,0,0)))
