function [numOfFilesProcessed,numOfDataPointsProcessed] = fr_site_met_database(wildCardPath,chanInd,chanNames,tableID,processProgressListPath,databasePath,fileType,time_shift,timeUnit,structPrefix,missingPointValue)
        ...
        try
            ...
            if ~isempty(structPrefix)
                ...
                for k=1:length(ClimateStats)
                    temp(k).(structPrefix) = ClimateStats(k);
            ...
            if ~isempty(findstr(databasePath,'\yyyy\'))
                ...
                for year_ind = years
                    ...
                    [k] = db_new_eddy(ClimateStats(one_year_ind),[],databasePathNew,0,[],timeUnit,missingPointValue);
                end
            else
