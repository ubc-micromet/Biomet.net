function [EngUnits, Header,tv] = fr_read_LGR1_file(fileName,assign_in,flagUBCGII_ASCII,varName,forceReadASCII)
...
%                                   Last modification:  Nov 18, 2018
%

% Revisions (last one first):
%
% Nov 15-18, 2018 (Zoran)
%   - Added the code to handle errors in LGR ASCII files that would prevent
%     this program from reading the entire file. 
%   - Added forceReadASCII to force re-creation of .mat file if needed.
% Dec 28, 2017 (Zoran)
...
%     name of the structure: 'LGR'
    ...
    % it instead
  try
    if forceReadASCII==1 || (isempty(strfind(fileName,'.mat')) && ~exist([fileName '.mat'],'file'))
        ...
        if isempty(strfind(Header.line2,'Time'))
            ...
            s_read = [];
            x_read = textscan(fid,lineFormat,'headerlines',0,'Delimiter',',','bufsize',20000);
            ...
            while ~isempty(x_read{1})
                ...
                ind =[];
tic                
                for j=1:length(x_read{1})
                    if isDateValid(x_read{1}(j),fileName)
                        % check if this row has data in all columns
                        % reject the index if it doesn't
                        if j <= length(x_read{end})
                            ind = [ind;j];
                        end
                    end
                end
...
function status = isDateValid(dateIn,fileName)

    status = true;
    if isempty(char(dateIn))
        status = false;
    else
        ...
        shortFileName = fileName(ind(end)+1:end);
        searchStr = sprintf('20%s/%s/%s',shortFileName(1:2),shortFileName(3:4),shortFileName(5:6));
        ind = strfind(dateIn,searchStr);
        if isempty(ind{1})
            % if string is not found the date format is wrong. Set the flag. 
            status = false;
        end
%         try
%             junk = datenum(dateIn);
