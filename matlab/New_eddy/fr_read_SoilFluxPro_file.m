...
Header = [];  % just a place holder to keep the same output parameters 
    ...
    try
        ...
        arg_default('timeUnits','5min');
        
        if isnumeric(timeUnits)
            strTimeUnits = sprintf('%dmin',timeUnits);
        else
            strTimeUnits = timeUnits;
        end
        ...
        tv = fr_round_time(tv_exact,strTimeUnits);       % this is the true time rounded to the nearest 30min
        ...
        for cntRows=1:numOfRows 
            ...
            try
                for cntVars=1:numOfVars             
                    outStruct(cntRows,1).(char(chambNames(cntRows))).(char(f1.Properties.VariableNames(cntVars))) ...
                            = EngUnits(cntRows,cntVars);  
                end
                outStruct(cntRows,1).(char(chambNames(cntRows))).sample_tv ...
                            =  tv_exact(cntRows);           
            catch ME
                fprintf('**** ERROR ==>  ');
                fprintf('%s\n',ME.message);
