function [EngUnits, Header,tv] = fr_read_SmartFlux_file(fileName,assign_in,varName)
...
%
%
% (c) Zoran Nesic                   File created:       Jul 11, 2018
%                                   Last modification:  Aug 30, 2022
%
...
%
% Aug 26, 2022 (Zoran)
%   - Extended flag variable separation from spikes_hf to all other flags
% Aug 24, 2022 (Zoran)
%   - Added separation of spikes_hf into multiple additional flags named
%     spikes_hf_1 to spikes_hf_9. Makes it easier to then filter dependent
%     data in the FirstStage.ini files. The original items is also kept (spikes_hf).
%
    ...
    try
        ...
        ind = find(double(Header.line1)==9);   % search for all tab delimeters (ascii = 9)
        N = length(ind)+1;          % this is the number of fields

        %Create: Header.var_names 
        tmp = textscan(Header.line1,'%s',N,'delimiter','\t'); % cell array of the var names
        tmp = tmp{1};
        for i=1:N-4
            newString = tmp{i+4};
        ...
        fid = fopen(fileName);
        % read the data
        EngUnits_tmp = textscan(fid,frmStr,'delimiter','\t','headerlines',2);
        ...
        % Extract time vector:
        tv=datenum(strcat(char(EngUnits_tmp{3}), ' '*ones(size(1,1)) ,char(EngUnits_tmp{4})));
