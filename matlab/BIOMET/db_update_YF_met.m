...
% function based on db_update_mpb_sites for the new Alberta hybrid poplar
% sites

% Revisions:
%
% Apr 3, 2018 (Zoran)
%   - added MET 5min data (YF_CR1000_MET.dat file processing)
%
...
for k=1:length(yearIn)
    for j=1:length(sites)
        ...
        % Progress list for YF_CR1000_1_MET (CR1000) logger
        eval(['progressList' char(sites(j)) '_MET_Pth = fullfile(pth_db,''' char(sites(j))...
            '_CR1000_1_MET_progressList_' num2str(yearIn(k)) '.mat'');']);        

        % Progress list for YF_CR1000_2_SOil_MET_30 (CR1000) logger
        eval(['progressList' char(sites(j)) '_Soil_30min_Pth = fullfile(pth_db,''' char(sites(j))...
            '_CR1000_2_Soil_MET_30_progressList_' num2str(yearIn(k)) '.mat'');']);
        ...
        % move YF_CR1000_1_MET.* files from csi_net/old  to  csi_net/old/yyyy
        % (those should be already in YF_CR1000_1_MET_#).yyyymmdd format, renamed at the
        % site already)
        datFiles = dir(fullfile(filePath,'YF_CR1000_1_MET.*'));
        for i=1:length(datFiles)
            if  ~datFiles(i).isdir 
                sourceFile      = fullfile(filePath,datFiles(i).name);
                destinationFile1 = fullfile(fullfile(filePath,num2str(yearIn(k))),datFiles(i).name);
        ...
        % move YF_CR1000_Soil_2_MET_30.* files from csi_net/old  to  csi_net/old/yyyy
        % (those should be already in YF_CR1000_2_MET_#).yyyymmdd format, renamed at the
        % site already)
        datFiles = dir(fullfile(filePath,'YF_CR1000_2_Soil_30min.*'));
        for i=1:length(datFiles)
            if  ~datFiles(i).isdir 
                sourceFile      = fullfile(filePath,datFiles(i).name);
                destinationFile1 = fullfile(fullfile(filePath,num2str(yearIn(k))),datFiles(i).name);
        ...
        switch char(sites(j))
            ...
            case 'YF'
                ...
                % Process YF_CR1000_MET data
                % *** ... ,outputPath,2,0,5) indicates 5 min data samples,
                % NOT the usual 30 min ****
                outputPath = fullfile(eval([char(sites(j)) '_ClimateDatabase_Pth']),'YF_CR1000_1_MET');
                eval(['[numOfFilesProcessed,numOfDataPointsProcessed] = fr_site_met_database(''D:\sites\' char(sites(j)) ...
                    '\CSI_net\old\' num2str(yearIn(k)) '\' char(sites(j)) '_CR1000_1_MET.' num2str(yearIn(k)) '*'',[],[],[],progressList' ...
                    char(sites(j)) '_MET_Pth,outputPath,2,0,5);'])             
                eval(['disp(sprintf(''' char(sites(j)) ...
                    ' YF_CR1000_1_MET:  Number of files processed = %d, Number of HHours = %d'',numOfFilesProcessed,numOfDataPointsProcessed))']);
                ...
                % Process YF_CR1000_2_Soil_MET_30 (CR1000) logger
                outputPath = fullfile(eval([char(sites(j)) '_ClimateDatabase_Pth']),'YF_CR1000_2_Soil_30Min');
                cTmp = ['[numOfFilesProcessed,numOfDataPointsProcessed] = fr_site_met_database(''D:\sites\' char(sites(j)) ...
                    '\CSI_net\old\' num2str(yearIn(k)) '\' char(sites(j)) '_CR1000_2_Soil_30Min.' num2str(yearIn(k)) '*'',[],[],[],progressList' ...
                    char(sites(j)) '_Soil_30min_Pth,outputPath,2);'];
                eval(cTmp)             
                eval(['disp(sprintf(''' char(sites(j)) ...
                    '_CR1000_2_Soil_30Min:  Number of files processed = %d, Number of HHours = %d'',numOfFilesProcessed,numOfDataPointsProcessed))']);
