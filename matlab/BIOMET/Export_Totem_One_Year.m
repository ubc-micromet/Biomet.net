function Export_Totem_One_Year(yearsIn)

% Revisions:
%
% July 16, 2022 (Zoran)
%   - Corrected units for radiation (were: W now: W/m2)
%
...
try
    ...
    for currentYear = yearsIn
        outputFileName = 'TotemData_%d.dat';
        ...
        %%
        formatX='%s, %6.2f, %6.2f, %6.2f, %6.2f, %6.2f, %6.2f, %6.2f, %6.2f, %6.2f \n';
        fid=fopen(pathOut,'w');
        %fid=2;
        if fid> 0
            ...
            fprintf(fid,'Date/Time,Ta (degC),RH (%s), precipitation (mm), global_radiation_main (W/m2), Tsoil_10 (degC), Tsoil_20 (degC), Tsoil_40 (degC),wind_speed (m/s), wind_direction (deg)  \n','%');
