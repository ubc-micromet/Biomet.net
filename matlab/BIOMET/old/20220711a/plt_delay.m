...
%
% Feb 2, 2020 (Zoran)
%   - this function used ubc's intercept.m function (modified version of
%     Matlab's own function that used a flag 'tv' - reasoning now lost in
%     time). I tested it on YF data (2015/16) and the standard Matlab version
%     (no 'tv' flag) returned identical results.  I preserved ubc's old
%     intercept as intersect_biomet_retro.m (just in case) and converted
%     this function to use intercept.m without 'tv' flag.
%
% Sept 19, 2012
%   -added HJP94
% Oct 3, 2011
...
case {'BS' 'CR' 'JP' 'PA'}
   ...
   [tv_in,indOut] = intersect(fr_round_hhour(tv),fr_round_hhour(tv_out));
   ...
   for i = 1:length(tv_days)
      ...
      delay_co2_set(i) = c.Delays.All(6+c.GillR2chNum); %#ok<*AGROW>
